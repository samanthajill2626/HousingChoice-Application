...
describe('isInlineMediaType', () => {
  it('accepts the allowlisted raster images (case-insensitive, trimmed)', () => {
...
describe('normalizeStoredMediaType', () => {
  it('keeps an allowlisted image (lowercased)', () => {
  ...
  it('collapses everything else to application/octet-stream', () => {
    ...
    expect(normalizeStoredMediaType('application/pdf')).toBe('application/octet-stream');
