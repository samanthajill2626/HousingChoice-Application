...
The infrastructure command initializes locked providers in owned disposable configuration mirrors, validates HCL and executes five deliberately broken module copies through named `mock_provider` Terraform assertions. Separately, its in-process source fixtures reject an active null default forwarding policy when the expected reference appears only in a trailing `#` or block comment, and preserve quoted and escaped strings such as `"/api/*"` while stripping comments outside strings. It also compares the shared dev/prod composition and runs backend-disabled init/validate in copies of both roots with their respective lockfiles; no root plan, apply, test or provisioner runs. Provider installation may need network access; an existing filesystem mirror containing the locked AWS/random providers can be supplied as the sole argument. Logs are under `.superpowers/maintenance-infra/`. It never loads live environment state or invokes a live plan/apply.
