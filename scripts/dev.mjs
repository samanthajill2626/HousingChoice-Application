...
// Local boot steps: DB container, tables, [seed only with --seeded], then run.
const localSteps = seedEnabled ? 4 : 3;
if (mode === 'local') {
  ...
  if (seedEnabled) {
    console.log(`dev — step 2/${localSteps}: tables`);
    await runTsx('app/scripts/db-create.ts');
    console.log(`dev — step 3/${localSteps}: seed data (--seeded)`);
    ...
    // gated to the localhost endpoint inside db-create.ts.)
    console.log(`dev — step 2/${localSteps}: tables (reset — empty DB)`);
