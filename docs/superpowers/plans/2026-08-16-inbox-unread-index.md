...
- [ ] **Step 3: Implement.** `incrementUnread` UpdateExpression becomes `'ADD unread_count :one SET unread_flag = :flag'` with `':flag': 'unread'` added to ExpressionAttributeValues (keep the existing ConditionExpression + ReturnValues). `resetUnread` becomes `'SET unread_count = :zero REMOVE unread_flag'`. `setRelayStatus`: its real signature takes `(conversationId, 'open' | 'closed')` (params typed at conversationsRepo.ts:724-728; round-2 review retracted the earlier expectedCurrent claim - do NOT widen the signature). In the `'closed'` branch ONLY, append `, unread_count = :zero` to its SET clause and add `REMOVE unread_flag` (one atomic write). The `'open'` branch is untouched. Extend the existing expression; do not rebuild it. `queryUnreadPage`:
...
- ROWFORCONVERSATION IS NOT EDITED AT ALL (round-2 decision, replacing the earlier dead-code-removal instruction that would have deleted the :533 unreadSum arm the hydration logic mirrors): the unread branch builds its rows in its OWN hydration function inside aggregateInbox, reusing the existing shared closures (`contactConversations`, `newestOf`, `latestMessageOf`, `placementLabel`) directly - so all/unknown keep rowForConversation byte-identical, and the now-unreachable `filter === 'unread'` arms inside it (:492, :533) get a one-line comment noting they are dead under the new unread path, nothing more.
...
- Inbox.tsx: widen the EXISTING error-state gate rather than duplicating JSX: the error block (copy "We couldn't load your inbox." at Inbox.tsx:116) renders when `status === 'error' || (status === 'ready' && rows.length === 0 && truncated)`. Do not invent new copy; the spec's 4.5 step 3 was aligned to this exact reuse.
