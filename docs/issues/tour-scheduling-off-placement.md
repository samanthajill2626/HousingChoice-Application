...
title: Move tour scheduling off the placement — tours happen during tenant Searching, before a placement exists
type: debt
severity: med
status: open
area: app
created: 2026-06-19
refs: documentation/STATUS-MODEL.md, app/src/lib/seedData.ts, app/src/routes/placements.ts
---
...
models tours **on the placement**: `tour_date`, `tour_history`, and the `tour_reminder`
deadline type all live on the `placements` entity (see the seed `placement-0001` and the
placement route/deadline logic). That means tour scheduling is attached to an entity that,
...
`tour_reminder` deadline off `placements`, decide where they live (likely a tenant↔unit
tour record or fields on the contact during Searching), and update any deadline/reminder
logic that currently keys on the placement. Reconcile the seed accordingly. Coordinate with
