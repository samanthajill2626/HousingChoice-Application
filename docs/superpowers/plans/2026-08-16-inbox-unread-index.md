...
- Test: `app/test/inboxUnreadParity.test.ts` (Create, Step 0), `app/test/inboxFeed.test.ts`, `app/test/inboxApi.test.ts`, `app/test/inboxGroups.test.ts`, `app/test/inboxEmail.test.ts`, `app/test/contactSoftDelete.test.ts`, `app/test/contactTriage.test.ts`, `app/test/inbox.integration.test.ts`
...
- CONTACT-ROW BODY: EXTRACT, DO NOT FORK - final contract (round-4): extract into one aggregateInbox-scoped helper
  `buildContactRow(contact, convs, maxConv, unreadSum, deleted): Promise<InboxRow | undefined>`
  containing EXACTLY: the deleted-resurfacing predicate block (:537-576), latestMessageOf(maxConv), the placement-label lookup, and the row literal (:583-597). SINGLE-CAUSE RETURN: undefined ONLY when resurfacing hides a deleted row. Everything else STAYS in rowForConversation: the relay/group skip, contact resolution, the no-contact/unknown branch, role + filter arms, the deleted computation, the emittedContacts.has guard, contactConversations + newestOf + the :527-528 identity guard, the :530-535 unreadSum + filter arms + deleted fast-path, AND `emittedContacts.add` - which MOVES CALLER-SIDE (:584 today, inside the extracted range): each caller adds its own dedupe entry when the helper returns a row (rowForConversation -> emittedContacts; the unread loop -> its seen-set; observably identical since the add already happened iff a row was returned). PHONE/EMAIL DERIVATION (round-4 finding: the literal reads `phone`/`email` bound at :472-473 from the ITERATED conv, invisible to a sibling helper): derive them INSIDE the helper from `maxConv.participant_phone`/`maxConv.participant_email`. This is PROVABLY byte-identical for the existing path: rowForConversation only emits when the iterated conv IS maxConv (the :527-528 identity guard), so conv-derived and maxConv-derived values coincide at every current emission; the unread path passes its hydrated maxConv. The unknown-candidate literal is NOT extracted (it reads the driving conv directly and the unread path builds it inline from the candidate's conversation - a ~10-line literal, acceptable duplication). relayRowFor (:620) and groupRowFor (:671) are reused as-is. Behavior-identity for all/unknown is proven by the untouched suites + Step 0 baseline; ADD the agreement test with the DISCRIMINATING fixture (round-4): a NAMELESS deleted contact owning a phone thread AND an email thread, unread on the NON-newest one, with a post-deletion inbound - rendered via BOTH paths (filter=all and filter=unread) yields deep-equal rows including the fallback-label name.
- BUDGET SEAM (plan-review B7): the unread branch consumes `deps.unreadWalkLimit ?? UNREAD_WALK_LIMIT` - the optional InboxRouterDeps field Task 5 INTRODUCED; do not re-declare it here.
- Page algorithm (spec 4.5, implement exactly):
  1. Loop: `collectUnreadRows({ maxRows: limit - rows.length, budget: remaining, startAfter, excludeContactIds: seen })` - `remaining` starts at `deps.unreadWalkLimit ?? UNREAD_WALK_LIMIT` (the Task 5 seam) and threads each call's `remainingBudget` forward; hydrate candidates (below); accumulate emitted contactIds into `seen` (WHETHER OR NOT hydration kept the row); SUM each call's `deletedProbes` into a request-level accumulator and fire `warnDeletedProbes(logger, total)` (the SAME shared unreadFeed.ts limiter Task 5 uses) once when the request total passes 25 (the sentinel is per-REQUEST; a per-collect threshold could never fire across many small collects); repeat until rows.length === limit, or `consumedAll`, or `remainingBudget === 0`.
  ...
  4. Relay-merge (:801-853) and group-source (:855-892) blocks: SKIPPED entirely under filter=unread (their rows now come from the collector). `groupsTruncated` never set here.
...
- [ ] **Step 2: Write the new failing route tests** (inboxFeed.test.ts unless noted):
  ...
  - probe-sentinel: fixture = 30 NON-qualifying deleted-contact unread threads (pre-deletion activity only) positioned newest, ahead of 3 visible normal unread, request limit 3 - the collector probes all 30 while hunting the 3 emittable candidates, so the request accumulator passes 25 and `warnDeletedProbes` fires once (spy on the shared limiter entry).
