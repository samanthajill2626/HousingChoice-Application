...
describe('systemStatus.getErrors — degradation + window', () => {
  ...
  it('surfaces a terminal Twilio delivery failure (now an ERROR) with its errorCode via the normal error query', async () => {
    // Terminal delivery failures log at level 50, so they arrive through the pino
    // ERROR query — no special always-on delivery query needed anymore.
    const config = deployedConfig();
    const fail = {
    ...
    const seam = fakeSeam({
      queryInsights: async (_groups, filterExpr) =>
        filterExpr === PINO_ERROR_INSIGHTS_FILTER ? [fail] : [],
    });
