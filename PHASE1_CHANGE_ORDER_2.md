...
4. **Ring-through rules (config, not architecture):** per-env/per-contact policy — e.g., active-case callers ring live, others go straight to voicemail + auto-text; optional business-hours window. Ship the rule seam with a simple default (everyone rings) and the active-case rule available behind config.
...
- **Process:** adversarial review reads doc §7.1 v2.16 itself; README deviations row only if you depart from this design; RUNBOOK gains the founder-facing behavior description (what she sees on Android vs iPhone, how to edit templates, how to enable ring-through rules).
