...
contract. The feature uses only Autolinker's public match API. It never uses the
HTML renderer, anchor builder, `link()` method, replacement callback, or the
runtime-only private `parseText()` method.
...
Keep the public Autolinker URL-only options module scoped rather than constructing
configuration during each React render. Before `Autolinker.parse` receives sender
text, replace only source `<` and `>` code units in a parser-only copy with the
same-length full-width equivalents U+FF1C and U+FF1E. This prevents the package's
HTML-aware parser path from interpreting sender text as tags or comments while
preserving every JavaScript offset. The rendered text and all destination candidates
still use the original source. Call `Autolinker.parse(maskedText, options)` with:
...
Then retain only results whose `type` is `url`. Use `getOffset()`, the matched-text
length, and `getUrlMatchType()` as the parser-owned source/range/classification
input, slicing the original source at that range. This keeps email addresses, phone
numbers, fuzzy IPs, bare single-label hosts, and protocol-relative local-only
targets outside the rendered-link input without treating a sender body as markup.
...
2. Prefix a parser-owned source beginning with `//` with `https:`. When
   `getUrlMatchType()` reports `tld`, prefix the exact matched source with
   `https://`; this parser classification covers both bare and `www` public-domain
   matches. Otherwise use the exact original matched source as the candidate. This
   preserves explicit HTTP(S) including literal `&amp;` query bytes and lets the
   final safety boundary reject an unsupported parsed scheme.
...
- literal `script`, anchor, and comment-shaped source text retaining parser-visible
  URLs as text-plus-link tokens rather than receiving HTML-aware suppression;
- literal `&amp;` query text remaining identical in an explicit HTTP(S) link's
  displayed and safety-checked destination source; and
