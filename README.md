...
| 2026-06-13 | Units address (M1.5) | §5 "Geocoded address" on the `units` table | `address` is stored as FREE TEXT; `jurisdiction` is an operator-set string. NO geocoding. | Kickoff explicitly scoped geocoding OUT of Phase 1. §5 already treats jurisdiction as a string the engine costs (not a coordinate), and matching never needs lat/long at this scale (§5.1). Geocoding can be added later (a stream-fed enrichment) with no key/contract change — units are flexible documents. |
