...
export function createDevRouter(deps: DevRouterDeps = {}): Router {
  ...
  router.post('/__dev/tour-reminders/tick', json(), async (req, res) => {
    ...
    // PRODUCTION SEMANTICS, exactly - no manualOnlyKinds override. The
    // deliberate divergence that used to live here existed only while
    // MANUAL_ONLY_REMINDER_KINDS held every auto-armed kind (2026-08-20 to
    // 2026-08-31); with the set emptied it would be a permanent dev/prod fork
    // for nothing, and its own text ordered its deletion. A green tick now
    // proves what the real poll does. NOTE the asymmetry with the
    // placement-nudge tick below: MANUAL_ONLY_NUDGE_KINDS is still full, so
    // that one keeps its override.
