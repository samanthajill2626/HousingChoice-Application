...
| `npm test` | Vitest across all workspaces. **Requires DynamoDB Local** (`npm run db:start`) and FAILS without it — 46 integration suites self-skip on an unreachable endpoint, so a Docker-less run used to omit ~631 tests and still exit 0. For a deliberate unit-only pass: `ALLOW_SKIP_DYNAMO_TESTS=1 npm test`, which prints exactly what it is giving up and is NOT a completion gate. Does NOT include the Playwright e2e suite (that's `npm run e2e`) | M0.1 |
| `npm run smoke` | Builds the app workspace and proves every import in the COMPILED output resolves the way plain `node dist/` will. Catches the bundler-vs-ESM resolution gap that tsx/esbuild hide (directory imports, exports-map violations) — a class no other gate can see. No Docker, no ports, ~1s. Required completion gate 3 of 4 | 2026-08 |
| `npm run e2e` | **UI end-to-end tests** (Playwright). Boots a hermetic stack via `scripts/e2e-session.mjs` (DynamoDB Local + app + worker + Vite, with `DEV_AUTH_ENABLED` on), drives a real browser through every spec, tears down. `reuseExistingServer` reuses a running session locally; CI boots fresh. Needs Docker. Full guide: [`e2e/README.md`](e2e/README.md) | P1 e2e |
...
| `npm run secrets:check` | Read-only drift report for the same keys (`-- dev` or `-- prod`): missing/differs/matches per key (masked), plus any unexpected extra params under `/hc/<env>/app/` (with a `→ secrets:prune` pointer). Exit 0 = in sync, 2 = drift, 1 = error | Phase 1 |
| `npm run secrets:prune` | Delete SSM params under `/hc/<env>/app/` (`-- dev` or `-- prod`) that are no longer in `.env.<env>` and not Terraform/deploy-managed — the orphans a removed key leaves behind (`secrets:push` only ever creates/updates, never deletes). **Dry run by default** (lists what it would delete, masked); pass `-- <env> --yes` to actually delete. Account-guarded; `MANAGED_BY_OTHERS` params are structurally protected | Phase 1 |
...
| `cloudfront` | The https entry point (price class 100, HTTP/2+3): origin = the EIP public DNS on HTTP:8080; `/api/*` + `/webhooks/*` and (for now) the default behavior all use CachingDisabled. **Custom domain (Change Order 3):** per-env ACM cert + alias for `app` / `dev.app` on `housingchoice.org` (SNI, min TLS 1.2), staged via the `custom_domain_phase` local; default `*.cloudfront.net` cert until cut over |
| `acm` | Per-stack custom-domain TLS cert (us-east-1, DNS-validated) for the CloudFront alias (Change Order 3). DNS validation records are hand-entered at Namecheap, so the apply is staged via `custom_domain_phase` and never deadlocks on the first run |
