...
All three changes are already in both `tables.auto.tfvars.json` files (GSIs regenerated in Phase A; the `unmatched_email` table in B3). **Online** operations - no recreate, **no backfill**: both GSI adds are sparse and populate as email participation is written (existing phone-only threads and contacts without an email never carry the indexed attribute, so they never index), and the new table starts empty. Post-merge on **dev**: `npm run plan -- dev` (review: two GSI adds + one new table with TTL) -> `npm run apply -- dev`. **Prod** rides M1.11 (`npm run plan -- prod` / `npm run apply -- prod` at the cutover).
