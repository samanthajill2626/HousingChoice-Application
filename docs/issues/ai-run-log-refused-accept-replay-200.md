...
**Why the wave did not fix it.** The fix extends the completed-row terminal
shape (persist the outcome or a refused disposition) - resolution-protocol
surgery of exactly the class three consecutive waves regressed on, and the
wave's scope discipline required reporting it instead of building it.

**Fix direction.** Persist the refusal on the completed row (outcome or a
dedicated disposition) and answer the identity-matched replay with the same
`409 suggestion_field_edited` the driving request gets.
