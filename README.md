...
| 2026-06-12 | Roles (M1.3) | users.role is `founder_admin \| va` (§5) | `admin \| va` | Operator preference; semantics identical (admin = the founder role). First login auto-provisions `va`; promotion only via `npm run user:role`. |
...
| `npm run user:role` | Set an EXISTING user's role: `-- <dev\|prod> <email> <admin\|va>` (account-guarded, direct DynamoDB on the `hc-<env>-users` table, `role_changed` audit event). Users auto-provision as `va` on first allowlisted Google login — this script is the only promotion path; the operator promotes the first admin with it | M1.3 |
