...
4. **Apply:** the same command WITHOUT `--dry-run`. `written` counts attachments whose message row write committed; an object whose copy landed but whose row write then failed stays octet-stream in the row and is simply finished by the next run.
