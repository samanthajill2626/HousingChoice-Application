...
- **`TWILIO_ACCOUNT_SID`, `TWILIO_API_KEY_SID`, `TWILIO_API_KEY_SECRET`, `TWILIO_AUTH_TOKEN`, `TWILIO_MESSAGING_SERVICE_SID`** for the account that RECEIVED the media. Dev and prod are different accounts, and the wrong one 404s on everything, which reads as retention loss.
- **`TWILIO_API_BASE_URL` must be UNSET.** It redirects every Twilio REST call - media metadata included - at the fake-Twilio host, and `loadConfig` only rejects it under `NODE_ENV=production`, which an operator running a `tsx` script is not. A shell that still carries it from an e2e session would read a fake account and report the whole population as aged out, so the script refuses outright rather than warning. The wrong-account case is caught too, but later and differently: each stored media URL names its owning Twilio account, and the first one that disagrees with `TWILIO_ACCOUNT_SID` ABORTS the run before any write.
