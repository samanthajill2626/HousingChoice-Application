...
- [ ] **Step 3: Implement.** `incrementUnread` UpdateExpression becomes `'ADD unread_count :one SET unread_flag = :flag'` with `':flag': 'unread'` added to ExpressionAttributeValues (keep the existing ConditionExpression + ReturnValues). `resetUnread` becomes `'SET unread_count = :zero REMOVE unread_flag'`. `setRelayStatus`: READ THE REAL SIGNATURE FIRST (conversationsRepo.ts:1786 - it takes an `expectedCurrent` status argument and guards the transition); when the TARGET status is `'closed'` - from EITHER `open` OR `connecting` as expectedCurrent - append `, unread_count = :zero` to its SET clause and add `REMOVE unread_flag` (one atomic write). Transitions to 'open' (reopen / connect-ready) are untouched. Extend the existing expression; do not rebuild it. Add a connecting->closed test case alongside the open->closed one. `queryUnreadPage`:
...
- DEAD-CODE REMOVAL, precisely (plan-review A3 - the plan's earlier :490/:530 cites were off): the `filter === 'unread'` conditionals inside rowForConversation (the unknown-branch early reject and the unreadSum reject added by main @39c1aa41 - grep `filter === 'unread'` in inbox.ts and remove EXACTLY those arms) become unreachable because filter=unread no longer enters the contact pager; remove them, KEEPING the `filter === 'unknown'` rejection and the deleted fast-path intact for the other filters.
...
- [ ] **Step 0: CAPTURE THE PARITY BASELINE BEFORE ANY inbox.ts EDIT** (plan-review: a builder mid-task cannot run 'pre-change' code). Write the new parity test file FIRST, running the CURRENT aggregateInbox against four fixtures (one contact, one unknown, one relay, one group under filter=unread), and serialize the returned row objects as literal expected JSON INTO the test (npx vitest run with a temporary console.log, paste, delete the log). Commit this passing baseline test on its own (`test(inbox): pin unread row-shape baseline`) so the rewrite diffs against pinned literals.
...
- Inbox.tsx: when `status === 'ready' && rows.length === 0 && truncated` render the EXISTING error-state block VERBATIM (its real copy is "We couldn't load your inbox." at Inbox.tsx:114 - reuse that exact markup + retry button) instead of the all-caught-up empty state. Do not invent new copy.
