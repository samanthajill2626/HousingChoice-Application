...
// a stable `code`. Feature agents import the endpoint functions (api/index.ts),
// not this file directly, but ApiError is re-exported there for `instanceof`.

/** A failed API call. `code` is the server's machine-readable { error } value
 *  (e.g. 'forbidden', 'cannot_demote_last_admin', 'push_not_configured') when
 *  present, else a synthetic code; `status` is the HTTP status (0 = network). */
