...
| Settings (Phone numbers) | our business number | `getByRole('heading', { name: 'Our number', level: 3 })` + `getByText(formatPhoneDisplay(APP_NUMBER))` - the read-only block EVERY authenticated user sees (route `/settings/numbers`, tab + `<h2>` both named "Phone numbers"). Unset renders the literal `Not set`. Scope negative assertions to `getByRole('region', { name: 'Phone numbers' })`: the admin-only pool block below it keeps the `Group text numbers` `<h3>` |
| Settings (System status) | "Sending from" pill | `getByText('Sending from', { exact: true })` - the configured `BUSINESS_PHONE_NUMBER` on the Go-live flags list. Pills are `<li>`s of two `<span>`s with NO accessible name, so match the label text and read the sibling state (a formatted number, or `Not set`). Do NOT match `/Not configured/` - that is the separate push-notifications pill |
