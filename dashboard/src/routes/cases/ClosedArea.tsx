...
// keyboard-toggleable; each row links to the case detail and shows the terminal
// stage label (and the lost-reason CATEGORY label for lost cases). Terminal
// cases are not draggable (they're done).
//
...
// CaseDetail pane, not list/card surfaces.
import { Link } from 'react-router-dom';
import { LOST_REASON_CATEGORY_LABELS, STAGE_LABELS, type CaseItem } from '../../api/index.js';
...
export function ClosedArea({ cases, tenantName, listingAddress }: ClosedAreaProps): React.JSX.Element | null {
        ...
        {cases.map((c) => {
          ...
          const reason =
            c.stage === 'lost' && c.lost_reason?.category !== undefined
              ...
              : '';
          return (
            <li key={c.caseId} className={styles.rowItem}>
              <Link to={`/cases/${c.caseId}`} className={styles.row}>
