...
created: 2026-08-24
refs: dashboard/src/routes/contact/Timeline.tsx:896, dashboard/src/routes/contact/Timeline.tsx:962, dashboard/src/routes/contact/Timeline.tsx:908, dashboard/src/routes/contact/Timeline.tsx:1011, dashboard/src/routes/contact/Timeline.tsx:1184
---

**Problem.** The message bubble's reveal is a `div` with an `onClick` and nothing
else - no `role`, no `tabIndex`, no `aria-expanded`
(`dashboard/src/routes/contact/Timeline.tsx:896-899`):
...
`revealed` state (`Timeline.tsx:962-988`), which is the only place the dashboard
ever names WHICH recipient a send did not confirm. The accessible summary this
branch added mitigates but does not erase the damage: the chip carries
`role="img"` plus an `aria-label` naming the unconfirmed recipients
(`Timeline.tsx:933-946` and the branch-0 chip at `:920-932`), so a screen-reader
user hears the summary - but they still cannot open the list to read the rows,
their times, or their per-leg reasons.
...
- a `Link` for a message intercepted from a now-closed relay group
  (`Timeline.tsx:908-914`), and
- the Retry button on a failed send (`Timeline.tsx:1011-1019`).
...
1. **A dedicated reveal control**, following the precedent that already exists a
   few hundred lines down THE SAME FILE: the call card's `Details` button
   (`Timeline.tsx:1184-1195`) is a real `<button>` with `aria-expanded={revealed}`
   and a per-card accessible name. Note this collides with locked decision 1 of
