...
>
> One drift is known already: the ticker described in S7 gained a clock-refresh on the rendered
> item set during the merge review, because the arming clock froze the first time the ticker
> disarmed. The `.superpowers/` artifacts this file cites - the adjudications, the review reports
> and the gate evidence - were gitignored and died with the worktree. They are archived outside
> the repo at `W:\tmp\_archive\per-recipient-delivery-2026-08-24\`.
